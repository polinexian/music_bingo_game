...
**Data flow**:
1. `data/songCatalog.json` (298 songs) → loaded at startup via `storage.ts` → IndexedDB cache
2. User selects songs → settings configured → `bingoCard.ts` generates unique cards → `pdfExport.ts` creates printable PDF
3. Host clicks play → `GameControls.tsx` calls `qqMusicPlayer.searchAndPlay()` → proxy searches NetEase → `<audio>` tag streams from NetEase CDN → `winCheck.ts` verifies bingo
4. State changes broadcast via `BroadcastChannel('music-bingo')` from Host → Display tab
